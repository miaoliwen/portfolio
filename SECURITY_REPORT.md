...
复审确认初版报告中的 P1/P2 项已落地：

- `parchment-diary` 已改为纯内存会话保存，启动时主动清理 `localStorage`/`sessionStorage` 残留，不再持久化日记明文。
- `vercel.json` 与 `1/server.cjs` 已配置完整安全响应头（CSP / HSTS / X-Frame-Options / nosniff / Referrer-Policy / Permissions-Policy）。
- 根项目与 `parchment-diary` 的开发脚本默认监听 `127.0.0.1`，新增 `dev:lan` 用于局域网联调。
- `1/app.js` 的图片渲染已从 `innerHTML` 字符串拼接改为 `createElement` + `appendChild`，消除 `src` 注入面。
